...
public class MinMax {
	...
	private List<MovimentoEstado> verificaCaminho(
		...
		// A lista "caminhos" possui todos os movimentos, o maior numero de
		// capturas é a maior lista
		int maiorNumeroDeCapturas = -1;
		Random r = new Random();
		// verifica se o jogo fico travado
		if (caminhos.isEmpty()) {
			System.out.println("vazio");
			Jogo.getInstance().desempate();
			
		}
		for (int i = 0; i < caminhos.size(); i++) {
			List<MovimentoEstado> list = caminhos.get(i);
			// Se a lista nao esta vazia e o valor for igual ao melhor valor
			// encontrado no MinMax
			if (!list.isEmpty() && valores.get(i) == v) {

				// Se for melhor caminho encontrado anteriormente com relacao ao
				// numero de capturas
				if (list.size() > maiorNumeroDeCapturas) {
					maiorNumeroCapturas = list;
					maiorNumeroDeCapturas = list.size();
					// Se encontrar um caminho com o numero de capturas igual ao
					// melhor caminho encontrado anteriormente, é escolhido um
					// caminho aleatoriamente
				} else if (list.size() == maiorNumeroDeCapturas
						&& r.nextInt(100) > 29) {// 70% de escolher outra
					maiorNumeroCapturas = list;
					maiorNumeroDeCapturas = list.size();
				}
	...
	public int MaxValue(Estado noAtual, int alfa, int beta, int nivel,
		...
		if (terminalTeste(nivel)
				|| (noAtual.getQtdPecaJ1() + noAtual.getQtdDamaJ1()) == 0
			...
			for (int j = 0; j < matriz.length; j++) {
				...
				if ((jogadorAtual == 1 && (matriz[i][j] == Estado.PECAJOGADOR1 || matriz[i][j] == Estado.DAMAJOGADOR1))
					...
					List<List<MovimentoEstado>> jogadas = Jogo.getInstance()
							.getTabuleiro()
							.caminhosDisponiveis(i, j, false, noAtual);
					...
					for (List<MovimentoEstado> list : jogadas) {
						...
						if (!list.isEmpty()) {
							...
							int min = MinValue(
									list.get(list.size() - 1).getT(), alfa,
									beta, nivel + 1, jogadorAtual == 1 ? 2 : 1);

							v = Math.max(v, min);
							// Se for o primeiro nivel adiciona a "caminho" para
							// evitar calcular de novo
							if (nivel == 0) {
							...
							// Se v for maior que o melhor valor retornado pelo
							// MIN
							if (v >= beta) {
								return v;
	...
	public int MinValue(Estado raiz, int alfa, int beta, int nivel,
		...
		if (terminalTeste(nivel)
				|| (raiz.getQtdPecaJ1() + raiz.getQtdDamaJ1()) == 0
		...
		for (int i = 0; i < matriz.length; i++) {
			for (int j = 0; j < matriz.length; j++) {
				...
				if ((jogadorAtual == 1 && (matriz[i][j] == Estado.PECAJOGADOR1 || matriz[i][j] == Estado.DAMAJOGADOR1))
					...
					List<List<MovimentoEstado>> jogadas = Jogo.getInstance()
							.getTabuleiro()
							.caminhosDisponiveis(i, j, false, raiz);
					...
					for (List<MovimentoEstado> list : jogadas) {
						...
						if (!list.isEmpty()) {
							...
							// Se v for menor que o melhor valor retornado pelo
							// MAX
							if (v <= alfa) {
								return v;
							}
